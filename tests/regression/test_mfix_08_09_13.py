"""Regression tests for MFIX-08, MFIX-09, MFIX-13 SDK fixes.

MFIX-08: stdout capture must use finally block for guaranteed restoration
MFIX-09: sys.exit() from CLI functions should return exit code, not crash SDK
MFIX-13: Exception handling should use specific types, not bare except
"""
import io
import re
import sys
...
class TestStdoutCapture:
    """MFIX-08: stdout capture must use finally block."""
    ...
    def test_stdout_restoration_guaranteed(self):
        ...
        # Find the wrapper function and check structure
        # The pattern should be: finally: ... sys.stdout = out_o
        # NOT: if not logger... sys.stdout = out_o (outside try/except)
...
class TestSysExitHandling:
    """MFIX-09: sys.exit() should be caught and converted."""
...
class TestExceptionSpecificity:
    """MFIX-13: Exception handling should be specific, not broad."""
